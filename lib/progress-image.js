...
async function drawHintCenter(image, width, height, text) {
  ...
  const pixel = Math.max(2, Math.floor(scale));
  const approxCharWidth = 3 * pixel + pixel;
  const totalWidth = String(text).length * approxCharWidth;
  const x = Math.max(0, Math.floor((width - totalWidth) / 2));
  const y = Math.max(0, Math.floor((height - 5 * pixel) / 2));
  drawFallbackText(image, text, x, y, hexToRgb(COLORS.emptyHint), pixel, 1);
}
...
async function renderYearMonthImage(todayObj, canvasWidth, canvasHeight) {
  ...
  for (let month = 1; month <= 12; month++) {
    ...
    const monthPixel = Math.max(1, Math.floor(scale * 0.75));
    drawFallbackText(
      image,
      monthLabels[monthIndex],
      Math.floor(blockX + dotStartX),
      Math.floor(blockY + monthTopInset),
      hexToRgb(COLORS.monthLabel),
      monthPixel,
      1
    );
