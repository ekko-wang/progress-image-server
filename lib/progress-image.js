...
// 本日模式样式 - 6 行 8 列 = 48 个点，每点代表半小时
const TODAY_STYLE = {
  canvasWidth: 1080,
  canvasHeight: 1080,
  maxDisplayDots: 48,
  columns: 8,        // 6 行 x 8 列 = 48 个点
  gridTopRatio: 0.15,
  gridWidthRatio: 0.80,
  textGapRatio: 0.08,
  colors: {
...
const MONTH_STYLE = {
  // 参考图：12个月「小点阵日历」布局（3列×4行）
  monthsColumns: 3,
  monthsRows: 4,
  monthsGridTopRatio: 0.18,
  monthsGridWidthRatio: 0.78,
  monthLabelGap: 14,
  monthBlockInnerPadding: 10,
  weekColumns: 7,
  weekRows: 6,
  dotMinSize: 6,
  dotMaxSize: 10,
  dotGap: 6,
  bottomTextYRatio: 0.80,
  colors: {
    background: '#151618',
    past: '#f2f2f2',
    current: '#f27a49',  // 橙色突出当天
    future: '#3f4044',
    textSecondary: '#9b9ca0'
  }
};
...
function weekdayIndexMondayFirst(d) {
  ...
  return (d.day() + 6) % 7; // Monday=0 ... Sunday=6
}

async function renderMonthCalendarImage({ yearStart, yearEnd, todayObj, leftDays, progressPercent }, canvasWidth, canvasHeight) {
  const style = MONTH_STYLE;
  const { colors } = style;
  const w = canvasWidth;
  const h = canvasHeight;

  const image = await Jimp.create(w, h, hexToRgb(colors.background));

  const gridWidth = Math.floor(w * style.monthsGridWidthRatio);
  const cellWidth = Math.floor(gridWidth / style.monthsColumns);
  const gridActualWidth = cellWidth * style.monthsColumns;
  const startX = Math.floor((w - gridActualWidth) / 2);
  const startY = Math.floor(h * style.monthsGridTopRatio);

  // 参考图顶部偏上且下方留白较大：月块区域高度取整体的约 58%
  const monthsAreaHeight = Math.floor(h * 0.58);
  const cellHeight = Math.floor(monthsAreaHeight / style.monthsRows);
  ...
  // 使用与其他模式相同的字体加载方式，确保 Vercel 上也能正常工作
  let labelFont = null;
  try {
  ...
  for (let m = 0; m < 12; m++) {
    const row = Math.floor(m / style.monthsColumns);
    const col = m % style.monthsColumns;
    const cellX = startX + col * cellWidth;
    const cellY = startY + row * cellHeight;

    // 月份标题
    if (labelFont) {
      const labelLayer = await createTintedTextLayer(monthNames[m], labelFont, colors.textSecondary);
      image.composite(labelLayer, cellX + style.monthBlockInnerPadding, cellY);
    }

    const monthStart = yearStart.month(m).startOf('month');
    const monthEnd = yearStart.month(m).endOf('month');
    const firstWeekday = weekdayIndexMondayFirst(monthStart);
    const daysInMonth = monthStart.daysInMonth();

    // 点阵区域
    const gridX = cellX + style.monthBlockInnerPadding;
    const gridY = cellY + style.monthLabelGap;

    const usableMonthWidth = cellWidth - style.monthBlockInnerPadding * 2;
    const step = Math.max(style.dotMinSize + style.dotGap, Math.floor(usableMonthWidth / style.weekColumns));
    const dotSize = Math.max(style.dotMinSize, Math.min(style.dotMaxSize, step - style.dotGap));
    const offsetWithinStep = Math.floor((step - dotSize) / 2);

    for (let day = 1; day <= daysInMonth; day++) {
      const d = monthStart.date(day);
      const idx = firstWeekday + (day - 1);
      const r = Math.floor(idx / style.weekColumns);
      const c = idx % style.weekColumns;
      if (r >= style.weekRows) continue;

      // 仅绘制本年范围内（理论上 monthStart/monthEnd 就在本年）
      if (d.isBefore(yearStart, 'day') || d.isAfter(yearEnd, 'day')) continue;

      let status = 'future';
      if (d.isBefore(todayObj, 'day')) status = 'past';
      else if (d.isSame(todayObj, 'day')) status = 'current';

      const x = gridX + c * step + offsetWithinStep;
      const y = gridY + r * step + offsetWithinStep;
      drawCircle(image, x, y, dotSize, colorMap[status]);
    }
  }

  // 底部文案：`xd left · y%` - 使用与其他模式相同的字体加载方式
  try {
    const leftLabel = `${leftDays}d left`;
    const rightLabel = ` · ${progressPercent}%`;
    ...
    const textX = Math.floor((w - totalTextWidth) / 2);
    const textY = Math.floor(h * style.bottomTextYRatio);
...
// 本日模式渲染函数 - 6 行 8 列 = 48 个点，每点代表半小时
async function renderTodayStyleImage(units, stats, canvasWidth, canvasHeight) {
  return renderStyleImage(units, stats, TODAY_STYLE, canvasWidth, canvasHeight);
}
...
async function generateProgressImage(query) {
  ...
  // 验证必传参数
  if (!viewType) {
    throw new Error('请传入 viewType（必传）。可选：today（本日）、day（年度按天）、week（年度按周）、month（年度按月）、range（起止日期，需同时传 startDate、endDate，格式 YYYYMMDD）、birthday（生日模式，需要 birthDate，格式 YYYYMMDD）');
  }
  ...
  if (viewType === 'range') {
    ...
    return renderMonthCalendarImage({ yearStart, yearEnd, todayObj, leftDays, progressPercent }, canvasWidth, canvasHeight);
  } else if (viewType === 'today') {
    // 本日模式：6 行 8 列 = 48 个点，每点代表半小时
    // 从 00:00 开始，每半小时一个点，共 48 个点覆盖 24 小时
    const dayStart = dayjs().startOf('day');
    const minutesPerDot = 30;
    const totalDots = 48; // 24 小时 * 2 = 48 个半小时
    const currentHour = todayObj.hour();
    const currentMinute = todayObj.minute();
    const currentTotalMinutes = currentHour * 60 + currentMinute;
    const currentDotIndex = Math.floor(currentTotalMinutes / minutesPerDot);

    for (let i = 0; i < totalDots; i++) {
      if (i < currentDotIndex) {
        timeUnits.push('past');
      } else if (i === currentDotIndex) {
        timeUnits.push('current');
      } else {
        timeUnits.push('future');
      }
    }

    // 本日模式：剩余半小时数 = 48 - 当前点索引 - 1
    const leftDots = totalDots - currentDotIndex - 1;
    const progressPercent = Math.max(0, Math.min(100, Math.round(((currentDotIndex + 1) / totalDots) * 100)));

    return renderTodayStyleImage(timeUnits, { leftDays: leftDots * 30, progressPercent }, canvasWidth, canvasHeight);
    ...
    throw new Error('viewType 仅支持：today（本日）、day（年度按天）、week（年度按周）、month（年度按月）、range（起止日期）、birthday（生日模式）');
  }
