 ...
 * @param {{ viewType: string, startDate?: string, endDate?: string }} query - viewType: day | week | range；range 时必传 startDate、endDate（YYYYMMDD）
 * @returns {Promise<Buffer>} PNG buffer
 */
async function generateProgressImage(query) {
  const { viewType, startDate, endDate } = query || {};
  let timeUnits = [];
  const today = dayjs().format('YYYY-MM-DD');

  if (!viewType) {
    throw new Error('请传入 viewType（必传）。可选：day（年度按天）、week（年度按周）、range（起止日期，需同时传 startDate、endDate，格式 YYYYMMDD）');
  }
  ...
  if (viewType === 'range') {
    ...
    throw new Error('viewType 仅支持：day（年度按天）、week（年度按周）、range（起止日期）');
  }
