...
async function renderMonthCalendarImage({ yearStart, yearEnd, todayObj, leftDays, progressPercent }, canvasWidth, canvasHeight) {
  ...
  try {
    defaultFont = await Jimp.loadFont(Jimp.FONT_SANS_16_BLACK);
  } catch (_err) {
    // 尝试其他默认字体
    try {
      defaultFont = await Jimp.loadFont(Jimp.FONT_SANS_32_BLACK);
    } catch (_err2) {
      defaultFont = null;
    }
  }
  ...
  for (let m = 0; m < 12; m++) {
    ...
    // 月份标题 - 使用默认字体直接绘制
    if (defaultFont) {
      image.print(defaultFont, cellX + style.monthBlockInnerPadding, cellY, {
        text: monthNames[m],
  ...
  try {
    ...
    if (font) {
      ...
      const leftWidth = Jimp.measureText(font, leftLabel);
      const rightWidth = Jimp.measureText(font, rightLabel);
      const totalTextWidth = leftWidth + rightWidth;
      ...
      image.print(font, textX + leftWidth, textY, {
        text: rightLabel,
        alignmentX: Jimp.HORIZONTAL_ALIGN_LEFT,
        alignmentY: Jimp.VERTICAL_ALIGN_TOP
      });
