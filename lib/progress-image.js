...
// 通用渲染函数 - 接受样式配置作为参数
async function renderStyleImage(units, stats, style) {
  const { canvasWidth, canvasHeight, maxDisplayDots, columns, gridTopRatio, gridWidthRatio, textGapRatio, colors } = style;
  const displayUnits = downsampleUnits(units, maxDisplayDots);
  const rows = Math.ceil(displayUnits.length / columns);
...
// 年度按周模式渲染函数
async function renderWeekStyleImage(units, stats) {
  return renderStyleImage(units, stats, WEEK_STYLE);
}
...
async function generateProgressImage(query) {
  ...
  if (viewType === 'range') {
    ...
    return renderWeekStyleImage(timeUnits, { leftDays, progressPercent });
