...
- **运行形态**：
  ...
  - 线上（EdgeOne Pages）：
    - `node-functions/api/index.js` 处理 `/api`（返回使用说明）。
    - `node-functions/api/dotpaper.js` 处理 `/api/dotpaper`（返回图片或提示页）。
    - `edgeone.json` 将 `/` 重定向到 `/api`，将 `/dotpaper` 重定向到 `/api/dotpaper`。
...
- **必填参数**
  - `viewType`：字符串，决定时间颗粒度和模式。
    ...
    - `month`：按“年度月份日历”显示（12 个月小点阵日历，参考图风格）。
    - `range`：自定义起止日期，每天一个点。
...
- 底部文案：`{leftDays}d left · {progressPercent}%`，其中前半段橙色，后半段灰色。
  - 兼容性说明：若部署环境缺失部分 Jimp 内置字体文件，会依次尝试一组“默认字体”（32 白体 → 16 白体 → 16 黑体）；仅当全部字体均加载失败时，才降级为不绘制文案，但图片仍正常返回（避免接口报错）。
